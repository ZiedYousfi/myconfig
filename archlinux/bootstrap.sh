...
update_config_disk() {
    ...
    if command -v jq &>/dev/null; then
        ...
        sed "s|/dev/sda|$INSTALL_DISK|g" "$CONFIG_DIR/user_configuration.json" > "$tmp_config"
    fi
...
run_archinstall() {
    ...
    reflector --country France,Germany,Switzerland --age 12 --protocol https --sort rate --save /etc/pacman.d/mirrorlist 2>/dev/null || true
...
setup_post_install() {
    ...
    local username
    username=$(grep -o '"username": "[^"]*"' "$CONFIG_DIR/user_credentials.json" | cut -d'"' -f4)
    ...
    cp /tmp/first-boot-setup.service "$mnt/etc/systemd/system/first-boot-setup.service"
